...
const BAR_COLORS = ["#035B71", "#00A2B9", "#E6A700", "#1E7F3E", "#9B4DCA", "#C4432B"];

/**
 * Rumus mengikuti sheet "Rekap anggaran" pada Excel yang diacu:
...
function useRekapData(rka, rab, laporan, tahun) {
  return useMemo(() => {
    ...
    return OPT.jenisKegiatanCsr.map((kegiatan) => {
      const anggaran = rka
        .filter((r) => r.jenisKegiatanCsr === kegiatan && r.tahun === tahun)
      ...
      <PageHeader
        ...
        right={
          <select
            ...
            style={{ padding: "9px 14px", borderRadius: 8, border: `1px solid ${T.border}`, background: T.inputBg, color: T.text, fontSize: 13.5, fontWeight: 600 }}
          >
            {YEAR_OPTIONS.map((y) => <option key={y} value={y}>Tahun {y}</option>)}
          </select>
        }
      />
      ...
      {!hasAnyData ? (
        ...
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 16 }}>
            ...
            <SummaryTile label="Penyerapan" value={`${(totalPenyerapan * 100).toFixed(1)}%`} tone="#8A6D00" />
          </div>
                ...
                <tbody>
                  {rows.map((r, i) => (
                    <tr key={r.kegiatan} style={{ background: i % 2 ? T.rowAlt : T.card }}>
                      ...
                      {r.monthly.map((v, mi) => (
                        <td key={mi} style={{ ...td, color: v ? T.text : T.muted }}>{v ? rupiah(v) : "-"}</td>
                      ))}
                      <td style={{ ...td, fontWeight: 700 }}>{rupiah(r.jumlahRealisasi)}</td>
                      <td style={{ ...td, color: r.saldo >= 0 ? T.success : T.danger, fontWeight: 700 }}>{rupiah(r.saldo)}</td>
                      <td style={td}><PenyerapanBadge value={r.penyerapan} /></td>
                    </tr>
                  ))}
                  <tr style={{ background: T.blueSoft, fontWeight: 700 }}>
                    ...
                    {Array.from({ length: 12 }, (_, mi) => (
                      <td key={mi} style={td}>{rupiah(rows.reduce((s, r) => s + r.monthly[mi], 0))}</td>
                    ))}
...
function SummaryTile({ label, value, tone }) {
  return (
    <div style={{ padding: "14px 16px", borderRadius: 12, background: T.card, border: `1px solid ${T.border}`, borderLeft: `3px solid ${tone}` }}>
...
function PenyerapanBadge({ value }) {
  ...
  return (
    <span style={{ display: "inline-block", padding: "2px 10px", borderRadius: 999, background: bg, color, fontSize: 11, fontWeight: 700 }}>
      {pct.toFixed(1)}%
    </span>
  );
}
        ...
        {rows.map((r, i) => {
          ...
          return (
            <g key={r.kegiatan}>
              ...
              <text x={gx + gw / 2} y={chartH + 14} fontSize="7" fill={T.muted} textAnchor="middle">{`Kegiatan ${i + 1}`}</text>
            </g>
          );
        })}
      </svg>
...
const th = {
  padding: "9px 10px", background: T.bg, borderBottom: `1px solid ${T.border}`,
  color: T.muted, fontSize: 10, fontWeight: 700, textTransform: "uppercase", textAlign: "center", whiteSpace: "nowrap",
};
const td = {
  padding: "9px 10px", borderBottom: `1px solid ${T.border}`, color: T.text, textAlign: "center", whiteSpace: "nowrap",
};
