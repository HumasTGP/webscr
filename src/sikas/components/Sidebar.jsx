...
function visibleTree(nodes, userRole, isAdmin) {
  return nodes
    ...
    .map((n) => (n.children ? { ...n, children: visibleTree(n.children, userRole, isAdmin) } : n))
...
// Kunci semua node (grup & subgrup) yang perlu kebuka supaya `activeKey` kelihatan.
function ancestorKeysOf(nodes, activeKey, trail = []) {
  for (const n of nodes) {
    if (n.key === activeKey) return trail;
    if (n.children) {
      const found = ancestorKeysOf(n.children, activeKey, [...trail, n.key]);
...
function containsActive(node, activeKey) {
  if (node.key === activeKey) return true;
  if (!node.children) return false;
  return node.children.some((c) => containsActive(c, activeKey));
}
...
// Item halaman (leaf) — style identik dengan tombol menu yang sudah ada,
// cuma indentasinya nambah sesuai kedalaman nesting.
function LeafButton({ node, depth, active, onSelect }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        alignItems: "center",
        gap: 10,
        padding: `8.5px 10px 8.5px ${10 + depth * 14}px`,
        borderRadius: 10,
        border: "none",
        cursor: "pointer",
...
// Header grup top-level — style ASLI dari sidebar sebelumnya (label kecil huruf
// besar + chevron), dipertahankan persis biar tampilan gak berubah.
function TopGroupHeader({ node, isOpen, isActiveDescendant, onToggle }) {
  return (
    <button
      onClick={onToggle}
      style={{
    ...
    >
      <span style={{
        ...
        letterSpacing: 0.8, color: isActiveDescendant ? T.blue : "#A9B3B6", textTransform: "uppercase",
      }}>
        {node.label}
      </span>
      <ChevronDown
        size={13}
        color={isActiveDescendant ? T.blue : "#C2CACC"}
        style={{
          flexShrink: 0,
...
// Subgrup bertingkat (mis. "NON PO" di dalam "Pembayaran", "Laporan" di dalam "Administrasi") —
// pakai bentuk pill yang sama kayak LeafButton (biar konsisten visual), tapi nambah chevron
// buka/tutup di kanan alih-alih langsung navigasi. Ini satu-satunya pola baru yang ditambahkan,
// dipakai khusus untuk nesting level 2+ yang sebelumnya belum ada di sidebar.
function SubGroupToggle({ node, depth, isOpen, isActiveAncestor, onToggle }) {
  const Icon = node.icon;
  return (
    <button
      onClick={onToggle}
      style={{
        ...
        cursor: "pointer",
        background: "transparent",
        color: isActiveAncestor ? T.blue : T.text,
        fontWeight: isActiveAncestor ? 600 : 500,
        fontSize: 13.2,
        fontFamily: font.body,
        textAlign: "left",
        transition: "background-color .15s ease, color .15s ease",
    ...
    >
      ...
      <ChevronDown
        size={13}
        color={isActiveAncestor ? T.blue : "#C2CACC"}
        style={{
          flexShrink: 0,
          transition: "transform .15s ease",
          transform: isOpen ? "rotate(180deg)" : "rotate(0deg)",
...
// Render rekursif satu node: leaf langsung jadi tombol, grup jadi toggle + anak-anaknya.
function NavNode({ node, depth, active, onSelect, openMap, toggleOpen }) {
  if (!node.children) {
    return <LeafButton node={node} depth={depth} active={active} onSelect={onSelect} />;
  }
  const isOpen = !!openMap[node.key];
  const isActiveAncestor = containsActive(node, active);
    ...
    return (
      <div>
        <TopGroupHeader
          node={node}
          isOpen={isOpen}
          isActiveDescendant={isActiveAncestor}
          onToggle={() => toggleOpen(node.key)}
        />
        {isOpen && (
...
export default function Sidebar({
  ...
  const tree = visibleTree(MENU_TREE, user.role, user.isAdmin);
  const flatLeaves = flattenLeaves(tree);
  const [hoveredKey, setHoveredKey] = useState(null);

  const [openMap, setOpenMap] = useState(() => {
    const ancestors = ancestorKeysOf(tree, active) || [];
    const initial = {};
    ancestors.forEach((k) => { initial[k] = true; });
    return initial;
  });

  // Kalau halaman aktif berubah (mis. lewat klik cepat, atau navigasi dari luar
  // sidebar) pastikan rantai grup induknya tetap kebuka.
  useEffect(() => {
    const ancestors = ancestorKeysOf(tree, active);
