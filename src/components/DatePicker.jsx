...
// Grid 6 minggu (42 sel) dari bulan yang lagi dilihat, termasuk padding
// tanggal bulan sebelum/sesudahnya.
...
/**
 * Kalender custom (full React, tanpa popup native browser) — dipakai
 * sebagai pengganti <input type="date"> supaya tidak kena bug Chrome di
 * mana native date-picker bisa "nyangkut" kebuka ulang terus-menerus.
 * value / onChange pakai format string "YYYY-MM-DD", sama seperti
 * <input type="date"> asli, jadi drop-in replacement.
 */
export default function DatePicker({ value, onChange, placeholder = "Pilih tanggal", disabled }) {
  ...
  const [pos, setPos] = useState({ top: 0, left: 0, width: 268 });
  ...
  const recalcPos = () => {
    ...
    // Kalau kepotong di kanan viewport, geser ke kiri biar tetap pas.
    let left = r.left;
  ...
  useEffect(() => {
    ...
    // Modal/isi halaman bisa discroll — ikutin posisi tombolnya biar gak lepas.
          ...
          >
            ...
            >
              ...
              <button
                ...
                style={{ ...linkBtnStyle, color: T.blue }}
              >
                Hari ini
              </button>
            </div>
