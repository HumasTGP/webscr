...
import { EKSPEDISI_OPT, ASAL_SURAT_OPT, JENIS_OPT } from "../../lib/siLapakPriokData";
...
export default function DataPaket({ paket, onProsesAmbil, onUpdate, onDelete }) {
  ...
  const filtered = paket.filter((p) => {
    ...
    const kodeUnik = p.jenis === "Surat" ? p.noSurat : p.noResi;
    const matchQ = p.namaPenerima.toLowerCase().includes(q.toLowerCase()) || (kodeUnik || "").toLowerCase().includes(q.toLowerCase());
    ...
    <div>
      {paket.length === 0 ? (
        <div style={{ textAlign: "center", padding: "40px 20px", color: T.muted }}>
          ...
          <div style={{ fontWeight: 700, fontSize: 13.5, color: "#5B4B00", marginBottom: 4 }}>Belum ada paket/surat</div>
          <div style={{ fontSize: 12 }}>Paket dan surat yang tercatat akan muncul di sini.</div>
        </div>
      ) : (
        <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-start" }}>
          <div style={{ flex: "1.3 1 340px" }}>
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cari nama, nomor resi, atau nomor surat" style={{ ...inputStyle, padding: "9px 12px", flex: 1 }} />
              <select value={jenisFilter} onChange={(e) => setJenisFilter(e.target.value)} style={{ ...inputStyle, padding: "9px 12px", width: 130, flexShrink: 0 }}>
                <option value="Semua">Semua jenis</option>
                {JENIS_OPT.map((j) => (<option key={j} value={j}>{j}</option>))}
              </select>
            </div>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead><tr><Th>Jenis</Th><Th>Nama</Th><Th>No. Resi/Surat</Th><Th>Status</Th></tr></thead>
              <tbody>
                {filtered.map((p) => (
                  <tr key={p.id} onClick={() => { setSelectedId(p.id); setEditMode(false); }} style={{ cursor: "pointer", background: selected?.id === p.id ? T.blueSoft : "transparent" }}>
                    <Td><JenisPill jenis={p.jenis} /></Td>
                    <Td>{p.namaPenerima}</Td>
                    <Td>{p.jenis === "Surat" ? p.noSurat : p.noResi}</Td>
                ...
                {!editMode ? (
                  <>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                      <div>
                        <JenisPill jenis={selected.jenis} />
                        <div style={{ fontSize: 13.5, fontWeight: 700, color: T.heading, marginTop: 6 }}>{selected.namaPenerima}</div>
                      </div>
                    ...
                    {selected.jenis === "Surat" ? (
                      <>
                        <DetailRow label="Asal surat" value={selected.asalSurat} />
                        <DetailRow label="Nomor surat" value={selected.noSurat} />
                        <DetailRow label="Perihal" value={selected.perihal} />
                      </>
                    ) : (
                      <>
                    ...
                    {selected.status === "Belum Diambil" && <button type="button" onClick={() => onProsesAmbil(selected)} style={{ width: "100%", marginTop: 12, padding: 10, borderRadius: 8, border: "none", background: T.navy, color: "#fff", fontWeight: 700, fontSize: 12.5, cursor: "pointer" }}>Proses ambil {selected.jenis.toLowerCase()}</button>}
                    {confirmDelete && (
                      <div style={{ marginTop: 12, background: "#FBE4E4", border: "1px solid #E8B4B4", borderRadius: 8, padding: 12 }}>
                        <div style={{ fontSize: 11.5, color: "#7A1F1F", marginBottom: 8 }}>Hapus data ini? Tindakan tidak dapat dibatalkan.</div>
                  ...
                  <>
                    <div style={{ fontSize: 12.5, fontWeight: 700, color: T.heading, marginBottom: 12 }}>Edit {draft.jenis.toLowerCase()}</div>
                    ...
                    {draft.jenis === "Surat" ? (
                      <>
                        <EditField label="Asal surat"><select style={inputStyle} value={draft.asalSurat} onChange={(e) => setDraft({ ...draft, asalSurat: e.target.value })}>{ASAL_SURAT_OPT.map((op) => (<option key={op} value={op}>{op}</option>))}</select></EditField>
                        <EditField label="Nomor surat"><input style={inputStyle} value={draft.noSurat} onChange={(e) => setDraft({ ...draft, noSurat: e.target.value })} /></EditField>
                        <EditField label="Perihal"><input style={inputStyle} value={draft.perihal} onChange={(e) => setDraft({ ...draft, perihal: e.target.value })} /></EditField>
                      </>
                    ) : (
                      <>
...
function JenisPill({ jenis }) { const isSurat = jenis === "Surat"; return <span style={{ display: "inline-block", fontSize: 9.5, fontWeight: 700, padding: "2px 8px", borderRadius: 20, background: isSurat ? "#EFE7FB" : "#E4EEFB", color: isSurat ? "#6B3FC4" : "#1D5FBF" }}>{jenis}</span>; }
