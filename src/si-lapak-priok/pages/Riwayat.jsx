...
function RiwayatTamu({tamu,onUpdate,onDelete}) {
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:10}}>{tamu.map((t)=><div key={t.id} style={{border:`1px solid ${T.border}`,borderRadius:10,padding:14,background:T.card}}>
    {editId===t.id ? <div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
      ...
      <div><label style={smallLabel}>Status</label><select style={inputStyle} value={draft.status||"Selesai"} onChange={(e)=>setDraft({...draft,status:e.target.value})}><option value="Sedang Berkunjung">Sedang Berkunjung</option><option value="Selesai">Selesai</option></select></div>
      <div><label style={smallLabel}>Jam keluar</label><input type="time" style={inputStyle} value={draft.jamKeluar||""} onChange={(e)=>setDraft({...draft,jamKeluar:e.target.value})}/></div>
    </div><div style={{display:"flex",gap:8}}><button type="button" onClick={saveEdit} style={primaryBtn}>Simpan</button><button type="button" onClick={()=>setEditId(null)} style={secondaryBtn}>Batal</button></div></div> : <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12}}><div style={{minWidth:0}}><div style={{fontSize:12.5,fontWeight:700,color:T.heading}}>{t.namaTamu}</div><div style={{fontSize:11,color:T.muted,marginTop:2,overflowWrap:"anywhere"}}>{t.tujuan} · menemui {t.pegawai}</div><div style={{fontSize:10.5,color:T.muted,marginTop:2}}>{t.tanggal} · masuk {t.jam}{t.jamKeluar?` · keluar ${t.jamKeluar}`:""} · {t.status||"Riwayat lama"}</div></div><div style={{display:"flex",gap:6,flexShrink:0}}><IconBtn onClick={()=>startEdit(t)} title="Edit"><Pencil size={13}/></IconBtn><IconBtn onClick={()=>setConfirmId(t.id)} title="Hapus" danger><Trash2 size={13}/></IconBtn></div></div>}
