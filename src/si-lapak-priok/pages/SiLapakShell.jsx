...
export default function SiLapakShell({ active, onSelect, onLogout, notificationItems = [], onOpenNotification, children }) {
  ...
  return (
    <div className="system-page-shell" style={{ "--blue":"#B79A00", "--navy":"#8C7600", "--blue-soft":"#FFF9D5", "--brand-yellow":"#FDEA6F" }}>
      <SiLapakSidebar active={active} onSelect={onSelect} onLogout={onLogout} notificationCount={notificationItems.length} />
      <div className="system-main">
        <div className="app-topbar" style={{ position:"sticky",top:0,zIndex:20,background:T.topbarBg,backdropFilter:"blur(8px)",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,minHeight:58,flexShrink:0 }}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
            <NotificationBell items={notificationItems} onOpenItem={onOpenNotification} onSeeAll={() => onSelect("notifikasi")} />
            <div style={{width:30,height:30,borderRadius:"50%",background:"#8C7600",color:"#fff",display:"grid",placeItems:"center",fontSize:10.5,fontFamily:font.display,fontWeight:800}}>SL</div><span className="hide-mobile" style={{fontSize:12.5,fontWeight:700,color:T.text}}>Si Lapak Priok</span>
          </div>
        </div>
