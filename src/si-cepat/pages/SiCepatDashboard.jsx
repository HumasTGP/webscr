...
function PengajuanForm({ user, onSubmit, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const submissionId = form.id || uid("MTR");
      const proposalFile = file
        ? (await uploadFile(file, "proposalSiCepat", { documentType: "PROPOSAL", recordId: submissionId, title: form.judulPengajuan })).file
        : null;
      onSubmit({ ...form, id: submissionId, nilaiDiajukan: Number(form.nilaiDiajukan) || 0, proposalFile });
    } catch (error) {
      setFileError(`Upload proposal gagal: ${error.message}`);
    }
  };
...
export default function SiCepatDashboard({ mitraList, user, setMitraList, notify, onGoto }) {
  ...
  const handleSubmit = (data) => {
    ...
    const newItem = {
      id: data.id || uid("MTR"), ...data,
      ownerAccountId: user?.id || "", ownerEmail: user?.email || "",
      status: MITRA_STATUS.MENUNGGU_HUMAS, createdAt: now,
      timeline: [{ status: MITRA_STATUS.MENUNGGU_HUMAS, tanggal: now, oleh: data.namaLembaga || "Pengaju", catatan: "Pengajuan diterima melalui Si Cepat" }],
    };
