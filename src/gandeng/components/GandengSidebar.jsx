...
export default function GandengSidebar({ active, onSelect, onLogout, user }) {
  ...
  return <div className="system-sidebar-wrap"><aside className="system-sidebar">
    ...
    <div style={{padding:"12px 14px 14px",borderTop:`1px solid ${T.border}`}}><button className="mobile-hide-sidebar" onClick={onLogout} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:9,padding:"9px 12px",borderRadius:10,border:"1px solid #F2B8B8",background:"#FFF5F5",color:"#CF0000",cursor:"pointer",fontSize:12,fontWeight:700}}><LogOut size={14}/><span>Keluar</span></button></div>
  </aside></div>;
}
