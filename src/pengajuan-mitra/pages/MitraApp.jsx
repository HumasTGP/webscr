...
import MitraAccountManagement from "./MitraAccountManagement";
...
export default function MitraApp({ mitraList, setMitraList, notify, onBackToPortal, user, onLogout }) {
  ...
  const renderContent = () => {
    switch (active) {
      case "account-management":
        return user?.isAdmin ? <MitraAccountManagement notify={notify} /> : <MitraDashboard mitraList={accountList} user={user} onGoto={setActive} />;
      case "dashboard": return <MitraDashboard mitraList={accountList} user={user} onGoto={setActive} />;
      case "pengajuan-baru":
      case "riwayat":
        return <div style={{ padding: "clamp(18px,3vw,28px) clamp(16px,4vw,32px)" }}><PengajuanMitraPage mitraList={accountList} setMitraList={setMitraList} user={user} notify={notify} onBackToPortal={null} /></div>;
      case "tracking": return <MitraTracking mitraList={accountList} />;
      case "bantuan": return <MitraBantuan />;
      default: return <MitraDashboard mitraList={accountList} user={user} onGoto={setActive} />;
    }
  };
  ...
  return <div style={{
    ...
    <MitraSidebar active={active} onSelect={setActive} onLogout={onLogout} onBackToPortal={onBackToPortal} user={user} />
